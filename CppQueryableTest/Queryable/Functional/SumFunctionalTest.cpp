...
TEST_F(SumFunctionalTest, SumDeque)
{
    ...
    const double sum = local.Sum([](const size_t value) { return static_cast<double>(value); });
    ASSERT_EQ(this->expectedSum, sum);
}
...
{
    constexpr size_t expected = 1110;
    const double sum = this->queryable.Where([](const size_t value) { return value > 10; }).Sum(
        [](const size_t value) { return static_cast<double>(value); });
