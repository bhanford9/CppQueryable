...
TEST_F(ExceptFunctionalTest, ExceptLocalFullInputEmpty)
{
    ...
    const std::vector<size_t> result = this->queryable.Except(local).ToVector();
...
{
    QueryableVector<size_t> a = BuildQueryable<size_t>(
        std::vector<size_t>({ 2, 5, 2, 5, 5, 5, 2, 2, 5 }));
    const std::vector<size_t> b = std::vector<size_t>({ 2, 5 });
    const std::vector<size_t> result = a.Except<>(b).ToVector();
...
{
    const PersonLibrary personLibrary;
    QueryableVector<Person> local = BuildQueryable<Person>(personLibrary.GetPeople());
    QueryableVector<Person> exceptions = BuildQueryable<Person>(
        std::vector<Person>(
            {
    ...
    const size_t originalLength = local.Count();
    const size_t afterExceptLength = originalLength - exceptions.Count();

    const std::vector<Person> result = local.Except(
        exceptions.ToVector(),
        [](const Person & a, const Person & b) { return a.GetName() < b.GetName(); }).ToVector();
...
TEST_F(ExceptFunctionalTest, ExceptDequeDequeDefault)
{
    ...
    const std::deque<size_t> result = local.Except(evens).ToDeque();
...
TEST_F(ExceptFunctionalTest, ExceptListDequeDefault)
{
    ...
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::list<size_t> result = local.Except(evens).ToList();
...
TEST_F(ExceptFunctionalTest, ExceptMapSetDefault)
{
    ...
    const std::set<std::pair<const size_t, std::string>> evens = BuildQueryable(
        this->queryableEvens.ToMap<size_t, std::string>(
            [](const size_t value) { return value; },
            [](const size_t value)
            {
    ...
    const std::set<std::pair<const size_t, std::string>> result = local.Except(evens).ToSet();
...
TEST_F(ExceptFunctionalTest, ExceptMapVectorDefault)
{
    ...
    const std::vector<std::pair<const size_t, std::string>> result = local.Except(evens).ToVector();
...
TEST_F(ExceptFunctionalTest, ExceptMultiSetDequeDefault)
{
    ...
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::multiset<size_t> result = local.Except(evens).ToMultiSet();
...
TEST_F(ExceptFunctionalTest, ExceptSetDequeDefault)
{
    ...
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::set<size_t> result = local.Except(evens).ToSet();
...
TEST_F(ExceptFunctionalTest, ExceptVectorDequeDefault)
{
    ...
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::vector<size_t> result = local.Except(evens).ToVector();
...
{
    constexpr int oddsOverTen = 2;
    QueryableDeque<size_t> local = BuildQueryable<size_t>(this->queryable.ToDeque());
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::deque<size_t> result = local.Where([](const size_t value) { return value > 10; }).
        Except(evens).ToDeque();

    ASSERT_EQ(oddsOverTen, result.size());
    for (const size_t value : result) { ASSERT_TRUE(value > 10); }
}

TEST_F(ExceptFunctionalTest, ExceptWhere)
{
    constexpr int oddsUnderTen = 3;
    QueryableDeque<size_t> local = BuildQueryable<size_t>(this->queryable.ToDeque());
    const std::deque<size_t> evens = this->queryableEvens.ToDeque();
    const std::deque<size_t> result = local.Except(evens).Where(
        [](const size_t value) { return value < 10; }).ToDeque();
