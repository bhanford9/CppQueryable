...
TEST_F(ContainsFunctionalTest, MapContains)
{
    ...
    ASSERT_TRUE(
        local.Contains(local.First([](const std::pair<const size_t, std::string> & kvp) { return kvp
            .first == 76 ; })));
...
{
    ASSERT_TRUE(
        this->queryable .Where([](const size_t value) { return value % 2 == 1; }) .Contains(1));
    ASSERT_FALSE(
        this->queryable .Where([](const size_t value) { return value % 2 == 1; }) .Contains(8));
}
