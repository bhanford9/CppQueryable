...
TEST_F(WhereFunctionalTest, WhereSetFemale)
{
  ...
  // std::set<Person, TCompare> people = Queryable<Person, TCompare>(this->queryable.ToSet(), [](Person a, Person b) { return a < b; })
  //   .Where([](Person p) { std::cout << "Where Set:\n" << p << std::endl; return p.GetGender() == Gender::Female; })
  //   .ToSet();
  //
  // for (Person p : people)
  //   ASSERT_TRUE(p.GetGender() == Gender::Female);
