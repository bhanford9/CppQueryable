...
public:
  ...
  {
    this->items = std::move(std::make_shared<SkipWhileQueryableSetData<TObj, TLessThan, TAllocator>>(
      std::move(this->items),
      std::move(condition)));
  }
  ...
  {
    this->items = std::move(std::make_shared<TakeWhileQueryableSetData<TObj, TLessThan, TAllocator>>(
      std::move(this->items),
      std::move(condition)));
  }
};
