...
class SelectQueryableDequeData : public QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>
{
private:
  std::function<TDestination(TSource)> selector;
  std::shared_ptr<QueryableData<TSource, std::deque, TSource, TSourceAllocator>> original;

public:
  SelectQueryableDequeData(
    std::shared_ptr<QueryableData<TSource, std::deque, TSource, TSourceAllocator>> && data,
    std::function<TDestination(TSource)> selector)
  ...
  SelectQueryableDequeData(
    const std::shared_ptr<QueryableData<TSource, std::deque, TSource, TSourceAllocator>> & data,
    std::function<TDestination(TSource)> selector)
    : QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>()
  {
    selector = selector;
  ...
  SelectQueryableDequeData(const SelectQueryableDequeData<TSource, TDestination, TDestinationAllocator, TSourceAllocator> & data)
    : QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>()
  {
  ...
  {
    return std::make_shared<SelectQueryableDequeData<TSource, TDestination, TSourceAllocator, TDestinationAllocator>>(*this);
  }
