...
class SelectQueryableListData : public QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>
{
private:
  std::function<TDestination(TSource)> selector;
  std::shared_ptr<QueryableData<TSource, std::list, TSource, TSourceAllocator>> original;

public:
  SelectQueryableListData(
    std::shared_ptr<QueryableData<TSource, std::list, TSource, TSourceAllocator>> && data,
    std::function<TDestination(TSource)> selector)
  ...
  SelectQueryableListData(
    const std::shared_ptr<QueryableData<TSource, std::list, TSource, TSourceAllocator>> & data,
    std::function<TDestination(TSource)> selector)
    : QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>()
  {
    selector = selector;
  ...
  SelectQueryableListData(const SelectQueryableListData<TSource, TDestination, TDestinationAllocator, TSourceAllocator> & data)
    : QueryableData<TDestination, std::vector, TDestination, TDestinationAllocator>()
  {
  ...
  {
    return std::make_shared<SelectQueryableListData<TSource, TDestination, TSourceAllocator, TDestinationAllocator>>(*this);
  }
  ...
  {
    // std::cout << "List Select Data begin" << std::endl;
  ...
  {
    // std::cout << "List Select Data end" << std::endl;
