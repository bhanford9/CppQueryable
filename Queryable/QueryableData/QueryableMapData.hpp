...
class QueryableMapData final : public QueryableData<
...
public:
    ...
    //   2. We want to supply both a key and a value to the add method
    ...
    QueryableMapData(const QueryableMapData & other)
        : QueryableData<TKey, std::map, std::pair<const TKey, TValue>, TValue, TCompare,
                        TAllocator>(other)
    {
    }

    QueryableMapData(QueryableMapData && other) noexcept
        : QueryableData<TKey, std::map, std::pair<const TKey, TValue>, TValue, TCompare,
    ...
    QueryableMapData(TCompare comparator = { }, TAllocator allocator = { })
        : QueryableData<TKey, std::map, std::pair<const TKey, TValue>, TValue, TCompare,
                        TAllocator>()
    {
        this->items = std::make_shared<std::map<TKey, TValue, TCompare, TAllocator>>(
            comparator,
            allocator);
    ...
    QueryableMapData & operator=(const QueryableMapData & other)
    {
    ...
    QueryableMapData & operator=(QueryableMapData && other) noexcept
    {
        if (this == &other) return *this;
        QueryableData<TKey, std::map, std::pair<const TKey, TValue>, TValue, TCompare,
    ...
    virtual ~QueryableMapData() override
    {
    }
    ...
    virtual std::pair<const TKey, TValue> & Get(const IteratorType type) override
    {
    ...
    virtual const std::pair<const TKey, TValue> & ConstGet(const IteratorType type) override
    {
        switch (type)
        {
        // case IteratorType::BeginForward: { *this->value = *this->beginIterator; return *this->value; }
